...
class SegmentTree:
    ...
    def find(self, value, lower_bound):
        def find_helper(l, h, i):

            if l >= array_size:
                return -1
            if h - l == 1:
                return l if self.tree_arr[i] >= value and l >= lower_bound else -1

            curr_value = self.tree_arr[i]
            mid = (l + h) // 2
            if curr_value < value: return -1

            res = find_helper(l, mid, 2 * i + 1) if mid >= lower_bound else -1
            if res == -1:
...
for _ in range(number_of_operations):
    ...
    if input_text.startswith("1"):
        ...
        _, x, l = map(int, input_text.split(" "))
        print(tree.find(x, l))